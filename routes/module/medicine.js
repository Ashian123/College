...
Medicine = require("../../models/medicine.model.js")(
  sequelize,
  Sequelize,
  DataTypes
);

medicine.get("/getdata", function (req, res) {
  console.log("Listing medicine data");

  Medicine
    .findAll({ where: { del: false } })
    .then((data) => {
      res.send(data);
    })
    .catch((err) => {
...
medicine.post("/ssr", (req, res, next) => {
...
medicine.post("/adddata", function (req, res) {
...
medicine.post("/handler", async (req, res) => {
  ...
  if (req.body.swtch == "get_medi_data") {
    console.log(req.body);
    Medicine.findOne({ where: { _id: req.body.m_id } })
      .then((data) => {
  ...
  if (req.body.swtch == "updateData") {
    ...
    Medicine.update(req.body, { where: { _id: req.body.uid } })
      .then((num) => {
        if (num == 1) {
          res.status(200).send("Update successfully.");
        } else {
          res.status(404).send({
            message: `Cannot update Medicine with id=${req.body.uid}. Maybe Tutorial was not found or req.body is empty!`,
          });
        }
      })
      .catch((err) => {
  ...
  if (req.body.swtch == "change_status") {
    ...
    Medicine.findByPk(req.body.uid)
      .then((data) => {
        if (data) {
          //res.send(data);
          Medicine.update(
  ...
  if (req.body.swtch == "del_formsdata") {
    Medicine.remove({ _id: req.body.data_id }, (err, formsdata) => {
      if (err) {
